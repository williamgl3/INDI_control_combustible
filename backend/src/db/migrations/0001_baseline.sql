-- Migración 0001 — línea base.
--
-- Captura tal cual el `schema.sql` histórico (idempotente, con `IF NOT
-- EXISTS`/`ON CONFLICT DO NOTHING`) para que el historial de migraciones
-- arranque limpio a partir de aquí. Instalaciones nuevas corren esto
-- primero; instalaciones ya existentes que ya tenían el esquema aplicado
-- a mano lo registran como aplicado sin volver a ejecutarlo con problema,
-- porque es idempotente (ver `src/db/migrate.ts`).
--
